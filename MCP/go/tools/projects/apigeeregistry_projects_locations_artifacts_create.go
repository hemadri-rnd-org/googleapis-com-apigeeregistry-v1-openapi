...
func Apigeeregistry_projects_locations_artifacts_createHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/%s/artifacts%s", cfg.BaseURL, parent, queryString)
...
func CreateApigeeregistry_projects_locations_artifacts_createTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v1_parent_artifacts",
		mcp.WithDescription("Creates a specified artifact."),
		mcp.WithString("parent", mcp.Required(), mcp.Description("Required. The parent, which owns this collection of artifacts. Format: `{parent}`")),
		mcp.WithString("artifactId", mcp.Description("Required. The ID to use for the artifact, which will become the final component of the artifact's resource name. This value should be 4-63 characters, and valid characters are /a-z-/. Following AIP-162, IDs must not have the form of a UUID.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apigeeregistry_projects_locations_artifacts_createHandler(cfg),
	}
}
