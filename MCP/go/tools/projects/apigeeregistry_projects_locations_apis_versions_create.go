...
func Apigeeregistry_projects_locations_apis_versions_createHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateApigeeregistry_projects_locations_apis_versions_createTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v1_parent_versions",
		mcp.WithDescription("Creates a specified version."),
		...
		mcp.WithString("apiVersionId", mcp.Description("Required. The ID to use for the version, which will become the final component of the version's resource name. This value should be 1-63 characters, and valid characters are /a-z-/. Following AIP-162, IDs must not have the form of a UUID.")),
		mcp.WithString("primarySpec", mcp.Description("Input parameter: The primary spec for this version. Format: projects/{project}/locations/{location}/apis/{api}/versions/{version}/specs/{spec}")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apigeeregistry_projects_locations_apis_versions_createHandler(cfg),
	}
}
